...
function initVertices() {
  ...
  makeSmoothSphere(); // Initialize the vertices for the smooth sphere 
  makeFacetedSphere(); // Initialize the vertices for the faceted sphere 

  // how many floats total needed to store all shapes including axes?
  var mySize = (gndVerts.length + axesVerts.length + smoothSphereVerts.length + facetedSphereVerts.length);

  // How many vertices total?
  // var nn = mySize / floatsPerVertex;
  // console.log('nn is', nn, 'mySiz is', mySize, 'floatsPerVertex is', floatsPerVertex);
  ...
  smoothSphereStart = i;
  for(j=0; j < smoothSphereVerts.length; i++, j++) {
    colorShapes[i] = smoothSphereVerts[j];
  }
  facetedSphereStart = i;
  for(j=0; j < facetedSphereVerts.length; i++, j++) {
    colorShapes[i] = facetedSphereVerts[j];
  }
...
// easily WITHOUT disrupting any other code, ever!
 ...
 `precision mediump float;
  ...
  this.vboContents = initVertices();						
  this.vboVerts = this.vboContents.length; // # of vertices held in 'vboContents' array
...
// Render current VBObox contents.
  ...
  pushMatrix(this.ModelMat);
    ...
    gl.drawArrays(gl.LINES, 	    // select the drawing primitive to draw,
                    // choices: gl.POINTS, gl.LINES, gl.LINE_STRIP, gl.LINE_LOOP, 
                    //          gl.TRIANGLES, gl.TRIANGLE_STRIP, ...
                    axesStart/floatsPerVertex, 								// location of 1st vertex to draw;
                    axesVerts.length/floatsPerVertex);		// number of vertices to draw on-screen.
  this.ModelMat = popMatrix();	

  // Draw Smooth Sphere 
  // pushMatrix(this.ModelMat);

  //   this.ModelMat.translate(0, -10, 5);
  //   this.ModelMat.rotate(g_angleNow0, 0, 0, 1);
  //   this.ModelMat.scale(5, 5, 5);
  ...
  //                   // choices: gl.POINTS, gl.LINES, gl.LINE_STRIP, gl.LINE_LOOP, 
  //                   //          gl.TRIANGLES, gl.TRIANGLE_STRIP, ...
  //                   smoothSphereStart/floatsPerVertex, 								// location of 1st vertex to draw;
  //                   smoothSphereVerts.length/floatsPerVertex);		// number of vertices to draw on-screen.
  // this.ModelMat = popMatrix();	
  ...
  //                   // choices: gl.POINTS, gl.LINES, gl.LINE_STRIP, gl.LINE_LOOP, 
  //                   //          gl.TRIANGLES, gl.TRIANGLE_STRIP, ...
  //                   facetedSphereStart/floatsPerVertex, 								// location of 1st vertex to draw;
  //                   facetedSphereVerts.length/floatsPerVertex);		// number of vertices to draw on-screen.
  // this.ModelMat = popMatrix();
...
function VBObox1() {
  ...
  // easily WITHOUT disrupting any other code, ever! 
    ...
    uniform vec3 u_LightColor1; // Light color
    uniform vec4 u_LightPosition1; // Position of the light source
    uniform vec3 u_AmbientLight1; // Ambient light color
    uniform vec3 u_LightSpec1;
    uniform vec4 u_eyePosWorld1; // Camera/eye location in world coords.
    uniform bool u_Blinn1;
    uniform vec3 u_kd1; // Phong Reflectance: diffuse
    uniform vec3 u_ka1; // Phong Reflectance: ambient
    uniform vec3 u_ke1; // Phong Reflectance: emissive
    uniform vec3 u_ks1; // Phong Reflectance: specular
    ...
    void main() {
       ...
       vec3 normal = normalize(vec3(u_NormalMatrix) * a_Norm1);
   
       // Calculate world coordinate of vertex
       vec4 vertexPosition = u_ModelMatrix * a_Pos1;
   
       // Calculate the light direction and make it 1.0 in length
       vec3 lightDirection = normalize(vec3(u_LightPosition1) - vec3(vertexPosition));
   
       vec3 eyeDirection = normalize(vec3(u_eyePosWorld1) - vec3(vertexPosition));
       ...
       float shiny = pow(spec, u_shiny);

       v_kd1 = u_kd1;
   
       // Calculate the color 
       vec3 diffuse = u_LightColor1 * nDotL * u_kd1;
       vec3 ambient = u_AmbientLight1 * u_ka1;
       vec3 emissive = u_ke1;
       vec3 specular = u_LightSpec1 * u_ks1 * shiny;

       vec4 color = vec4(a_Norm1, 1.0); // Sphere color
      // vec4 color = vec4(0.2, 1.0, 0.2, 1.0); // Sphere color

       // Add the surface colors due to diffuse reflection and ambient reflection
       v_Colr1 = vec4(diffuse + ambient + emissive + specular, color.a);
   ...
   `precision mediump float;
    varying vec4 v_Colr1;
    void main() {
      // gl_FragColor = vec4(v_Colr1.z, v_Colr1.z, v_Colr1.z, 1.0);
      gl_FragColor = v_Colr1;

    }`;
	
    this.vboContents = makeSmoothSphere();
    // this.vboContents = makeFacetedSphere();
    ...
    this.vboFcount_a_Norm1 = 3;   // # of floats for this attrib (nx,ny,nz values)
    ...
    this.u_MvpMatrixLoc;						// GPU location for u_MvpMat uniform
    this.u_NormalMatrixLoc;						// GPU location for u_NormalMat uniform
    this.u_LightColorLoc;          	// GPU location for u_LightColor1
    this.u_LightPositionLoc; // GPU location for u_LightPosition1
    this.u_AmbientLightLoc;  // GPU location for u_AmbientLight1
    this.u_LightSpecLoc; 
...
//  uniform-variables (for switchToMe(), adjust(), draw(), reload(), etc.)
  ...
  this.u_LightColorLoc = gl.getUniformLocation(this.shaderLoc, 'u_LightColor1');
  if (!this.u_LightColorLoc) { 
    console.log(this.constructor.name + 
    						'.init() failed to get GPU location for u_LightColor1 uniform');
    return;
  }
  this.u_LightPositionLoc = gl.getUniformLocation(this.shaderLoc, 'u_LightPosition1');
  if (!this.u_LightPositionLoc) { 
    console.log(this.constructor.name + 
    						'.init() failed to get GPU location for u_LightPosition1 uniform');
    return;
  }
  this.u_AmbientLightLoc = gl.getUniformLocation(this.shaderLoc, 'u_AmbientLight1');
  if (!this.u_AmbientLightLoc) { 
    console.log(this.constructor.name + 
    						'.init() failed to get GPU location for u_AmbientLight1 uniform');
    return;
  }
  this.u_LightSpecLoc = gl.getUniformLocation(this.shaderLoc, 'u_LightSpec1');
  if (!this.u_LightSpecLoc) { 
    console.log(this.constructor.name + 
    						'.init() failed to get GPU location for u_LightSpec1 uniform');
    return;
  }
  this.u_eyePosWorldLoc = gl.getUniformLocation(this.shaderLoc, 'u_eyePosWorld1');
  if (!this.u_eyePosWorldLoc) { 
    console.log(this.constructor.name + 
    						'.init() failed to get GPU location for u_eyePosWorld1 uniform');
    return;
  }
  this.u_isBlinnLoc = gl.getUniformLocation(this.shaderLoc, 'u_Blinn1');
  if (!this.u_isBlinnLoc) { 
    console.log(this.constructor.name + 
    						'.init() failed to get GPU location for u_Blinn1 uniform');
    return;
  }
  this.u_kdLoc = gl.getUniformLocation(this.shaderLoc, 'u_kd1');
  if (!this.u_kdLoc) { 
    console.log(this.constructor.name + 
    						'.init() failed to get GPU location for u_kd1 uniform');
    return;
  }
  this.u_kaLoc = gl.getUniformLocation(this.shaderLoc, 'u_ka1');
  if (!this.u_kaLoc) { 
    console.log(this.constructor.name + 
    						'.init() failed to get GPU location for u_ka1 uniform');
    return;
  }
  this.u_keLoc = gl.getUniformLocation(this.shaderLoc, 'u_ke1');
  if (!this.u_keLoc) { 
    console.log(this.constructor.name + 
    						'.init() failed to get GPU location for u_ke1 uniform');
    return;
  }
  this.u_ksLoc = gl.getUniformLocation(this.shaderLoc, 'u_ks1');
  if (!this.u_ksLoc) { 
    console.log(this.constructor.name + 
    						'.init() failed to get GPU location for u_ks1 uniform');
    return;
  }
  this.u_shinyLoc = gl.getUniformLocation(this.shaderLoc, 'u_shiny');
  if (!this.u_shinyLoc) { 
    console.log(this.constructor.name + 
    						'.init() failed to get GPU location for u_shiny uniform');
    return;
  }
...
// the GPU; and (if needed) update each attribute's stride and offset in VBO.
  ...
  this.MvpMatrix.set(g_worldMat).multiply(this.ModelMatrix);
  ...
  gl.uniform4f(this.u_LightPositionLoc, g_lightPosn[0], g_lightPosn[1], g_lightPosn[2], 1.0);

  // Set the ambient light
  gl.uniform3f(this.u_AmbientLightLoc, 0.2, 0.2, 0.2);

  gl.uniform3f(this.u_LightSpecLoc, 1.0, 1.0, 1.0);		// Specular
  
  // Set the Phong materials' reflectance:
	gl.uniform3f(this.u_keLoc, g_material.K_emit[0], g_material.K_emit[1], g_material.K_emit[2]);				// Ke emissive
  gl.uniform3f(this.u_kaLoc, g_material.K_ambi[0], g_material.K_ambi[1], g_material.K_ambi[2]);     // Ka ambient
  gl.uniform3f(this.u_kdLoc, g_material.K_diff[0], g_material.K_diff[1], g_material.K_diff[2]);     // Kd	diffuse
  gl.uniform3f(this.u_ksLoc, g_material.K_spec[0], g_material.K_spec[1], g_material.K_spec[2]);     // Ks specular
  gl.uniform1f(this.u_shinyLoc, g_material.K_shiny);                 // Kshiny shinyness exponent

  gl.uniform1i(this.u_isBlinnLoc, g_blinn);           // Use Blinn-Phone Lighting 
  
  // Pass the eye position to u_eyePosWorld
	gl.uniform4f(this.u_eyePosWorldLoc, eye[0], eye[1], eye[2], 1.0);


//  this.ModelMatrix.rotate(g_angleNow0, 0, 0, 1);	// -spin drawing axes,
  // this.ModelMatrix.translate(0.0, 2.0, 0);						// then translate them.
  //  Transfer new uniforms' values to the GPU:-------------
  // Send  new 'ModelMat' values to the GPU's 'u_ModelMat1' uniform: 
  ...
  // Send  new 'NormalMat' values to the GPU's 'u_NormalMat1' uniform: 
...
// easily WITHOUT disrupting any other code, ever!
  ...
  void main() {
      ...
      v_Normal1 = normalize(vec3(u_NormalMatrix) * a_Norm1);
      v_kd1 = u_kd1;
  }`;
 ...
 `precision mediump float;
  ...
  void main() {
      ...
      vec3 lightDirection = normalize(vec3(u_LightPosition1) - vec3(v_Pos1));
  
      float nDotL = max(dot(lightDirection, normal), 0.0);
  
      vec3 eyeDirection = normalize(vec3(u_eyePosWorld1) - vec3(v_Pos1));
      ...
      // Calculate the color 
      vec3 diffuse = u_LightColor1 * nDotL * v_kd1;
      vec3 ambient = u_AmbientLight1 * u_ka1;
      vec3 emissive = u_ke1;
      vec3 specular = u_LightSpec1 * u_ks1 * shiny;
...
VBObox2.prototype.init = function() {
  ...
  // the GPU; and (if needed) update each attribute's stride and offset in VBO.
    ...
    // Set the light color (white)
    gl.uniform3f(this.u_LightColorLoc, 1.0, 1.0, 1.0);
    // Set the light direction (in the world coordinate)
    gl.uniform4f(this.u_LightPositionLoc, g_lightPosn[0], g_lightPosn[1], g_lightPosn[2], 1.0);
    // Set the ambient light
    gl.uniform3f(this.u_AmbientLightLoc, 0.2, 0.2, 0.2);
  ...
  // Send commands to GPU to select and render current VBObox contents.
    ...
    //   this.MvpMatrix.rotate(g_angleNow0, 0, 0, 1);
    //   gl.uniformMatrix4fv(this.u_MvpMatrixLoc,	// GPU location of the uniform
    //   false, 				// use matrix transpose instead?
    //   this.MvpMatrix.elements);	// send data from Javascript.
    //   this.ModelMatrix.rotate(g_angleNow0, 0, 0, 1);
...
function VBObox3() {
  ...
  VBObox3.prototype.init = function() {
    ...
    // Send commands to GPU to select and render current VBObox contents.
      ...
      //   this.MvpMatrix.elements);	// send data from Javascript.
      //   // this.ModelMatrix.rotate(g_angleNow0, 0, 0, 1);
      //   // gl.uniformMatrix4fv(this.u_MoMatrixLoc,	// GPU location of the uniform
      //   // false, 				// use matrix transpose instead?
      //   // this.MvpMatrix.elements);	// send data from Javascript.
      ...
      // this.ModelMatrix.setIdentity();
      // this.ModelMatrix.translate(10.0, 10.0, 1.0);
      // Apply transformations directly within the draw function.
      // var modelMatrix = new Matrix4(); // Consider using a temporary matrix if needed.
      // modelMatrix.setTranslate(10.0, 10.0, 1.0); // Apply translation.
      // modelMatrix.rotate(g_angleNow0, 0, 0, 1); // Apply rotation.
      // this.ModelMatrix.rotate(g_angleNow0/10.0, 0, 0, 1);
      // this.ModelMatrix.translate(10.0, 10.0, 1.0);
      
      pushMatrix(this.MvpMatrix);
        this.MvpMatrix.translate(7.0, 7.0, 1.0);
        gl.uniformMatrix4fv(this.u_MvpMatrixLoc,	// GPU location of the uniform
          false, 				// use matrix transpose instead?
          this.MvpMatrix.elements);	// send data from Javascript.
        gl.drawArrays(gl.TRIANGLE_STRIP,		    // select the drawing primitive to draw:
        ...
        this.MvpMatrix.translate(0.0, 0.0, 1.0);
        this.MvpMatrix.rotate(g_angleNow1, 1, 0, 0);
        this.MvpMatrix.scale(2.0, 0.5, 0.5);
        // this.MvpMatrix.rotate(g_angleNow0, 0.0, 0.0, 1.0);
        gl.uniformMatrix4fv(this.u_MvpMatrixLoc,	// GPU location of the uniform
          false, 				// use matrix transpose instead?
          this.MvpMatrix.elements);	// send data from Javascript.
        gl.drawArrays(gl.TRIANGLE_STRIP,		    // select the drawing primitive to draw:
        ...
        pushMatrix(this.MvpMatrix);
          this.MvpMatrix.translate(1.0, 0.0, 1.0);
          this.MvpMatrix.rotate(g_angleNow1, 1, 0, 0);
          this.MvpMatrix.scale(0.5, 1.5, 0.5);
          // this.MvpMatrix.rotate(g_angleNow0, 0.0, 0.0, 1.0);
          gl.uniformMatrix4fv(this.u_MvpMatrixLoc,	// GPU location of the uniform
            false, 				// use matrix transpose instead?
            this.MvpMatrix.elements);	// send data from Javascript.
      ...
      pushMatrix(this.MvpMatrix);
        ...
        this.MvpMatrix.translate(14.0, -14.0, 5.0);
        ...
        this.MvpMatrix.rotate(g_angleNow0, 1, 0, 0);

        pushMatrix(this.MvpMatrix);
          this.MvpMatrix.translate(0.0, 0.0, 2.0);
          this.MvpMatrix.scale(1.0, 1.0, 2.0);
          gl.uniformMatrix4fv(this.u_MvpMatrixLoc,	// GPU location of the uniform
            false, 				// use matrix transpose instead?
            this.MvpMatrix.elements);	// send data from Javascript.
          gl.drawArrays(gl.TRIANGLE_STRIP,		    // select the drawing primitive to draw:
        ...
        pushMatrix(this.MvpMatrix);
          this.MvpMatrix.translate(0.0, 0.0, 4.0);
          this.MvpMatrix.scale(1.0, 2.0, 1.0);
          this.MvpMatrix.rotate(g_angleNow2, 0, 0, 1);
          gl.uniformMatrix4fv(this.u_MvpMatrixLoc,	// GPU location of the uniform
            false, 				// use matrix transpose instead?
            this.MvpMatrix.elements);	// send data from Javascript.
          gl.drawArrays(gl.TRIANGLE_STRIP,		    // select the drawing primitive to draw:
      ...
      pushMatrix(this.MvpMatrix);
        ...
        this.MvpMatrix.translate(-21.0, 21.0, 4.0);

        pushMatrix(this.MvpMatrix);
          this.MvpMatrix.scale(0.5, 0.5, 3.0);
          gl.uniformMatrix4fv(this.u_MvpMatrixLoc,	// GPU location of the uniform
            false, 				// use matrix transpose instead?
            this.MvpMatrix.elements);	// send data from Javascript.
          gl.drawArrays(gl.TRIANGLE_STRIP,		    // select the drawing primitive to draw:
        ...
        pushMatrix(this.MvpMatrix);
          this.MvpMatrix.translate(0.0, 0.0, 3.0);
          this.MvpMatrix.scale(2.0, 0.5, 0.5);
          gl.uniformMatrix4fv(this.u_MvpMatrixLoc,	// GPU location of the uniform
            false, 				// use matrix transpose instead?
            this.MvpMatrix.elements);	// send data from Javascript.
        ...
        pushMatrix(this.MvpMatrix);
          this.MvpMatrix.translate(0.0, 0.0, 4.0);
          this.MvpMatrix.scale(2.0, 3.0, 1.0);
          this.MvpMatrix.rotate(g_angleNow2, 0, 0, 1);
          gl.uniformMatrix4fv(this.u_MvpMatrixLoc,	// GPU location of the uniform
            false, 				// use matrix transpose instead?
            this.MvpMatrix.elements);	// send data from Javascript.
